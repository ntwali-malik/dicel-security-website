        ...
        <div>
            ...
            <div className="container-fluid page-header py-5">
                <div className="container text-center py-5">
                    <h1 className="display-2 text-white mb-4 animated slideInDown">Gallery</h1>
                </div>
            </div>
            {/* <!-- Page Header End --> */}
            {/* <!-- Gallery Section Start --> */}
            <div className="container py-5 wow fadeIn" data-wow-duration="1s">
                <div className="row justify-content-center mb-4">
                    <div className="col-lg-8 text-center wow fadeInDown" data-wow-delay=".1s">
                        <h2 className="fw-bold">Our Gallery</h2>
                        <p className="text-muted mb-0">A curated showcase of premium security solutions, crafted for elegance and reliability.</p>
                    </div>
                </div>

                {/* Filters */}
                <div className="row mb-4">
                    <div className="col-12 d-flex justify-content-center gap-2 flex-wrap wow fadeInUp" data-wow-delay=".15s">
                        {[
                            { key: 'all', label: 'All' },
                            { key: 'cctv', label: 'CCTV' },
                            ...
                            <button
                                ...
                                className={`btn btn-sm ${activeFilter === filter.key ? 'btn-primary' : 'btn-outline-primary'}`}
                            >
                                {filter.label}
                            </button>
                        ))}
                ...
                <div className="row g-3 g-md-4">
                    {filteredImages.map((img, idx) => {
                        const delay = `${0.05 * (idx % 8)}s`
                        return (
                            <div key={`${img.src}-${idx}`} className="col-6 col-md-4 col-lg-3 wow fadeInUp" data-wow-delay={delay}>
                                <div className="position-relative overflow-hidden rounded-3 shadow-sm gallery-item" style={{ cursor: 'pointer', transition: 'transform .35s ease, box-shadow .35s ease' }} onClick={() => openLightbox(img)} onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-4px) scale(1.02)'; e.currentTarget.style.boxShadow = '0 1rem 2rem rgba(0,0,0,.15)'; }} onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = 'var(--bs-box-shadow-sm)'; }}>
                                    <img src={img.src} alt="Gallery item" className="img-fluid w-100" style={{ objectFit: 'cover', height: '200px', transition: 'transform .35s ease' }} />
                                </div>
                            </div>
                        )
                    })}
